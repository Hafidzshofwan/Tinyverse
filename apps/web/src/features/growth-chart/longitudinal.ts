...
export interface GrowthRecord {
  ...
  catatan?: string;
  createdAt: number;
  /** Cara ukur panjang/tinggi saat pemeriksaan ini. Rekam lama tidak punya. */
  caraUkur?: CaraUkurPanjang;
  /** Versi skema perhitungan z-score. Rekam tanpa penanda ini dihitung ulang. */
  zVersi?: number;
}
...
//
// Isi tabelnya kini berada di ./bbpbTabel supaya interpretasi.ts bisa
// memakai angka yang sama tanpa ikut menarik React dan Firebase dari
// berkas ini. Diekspor ulang di bawah agar pemanggil lama tidak berubah.
// ==========================================
...
/**
 * Versi skema perhitungan z-score yang tersimpan pada tiap rekam.
 *
 * Versi 1 (tanpa penanda) menyimpan bbtbZ hasil rumus karangan
 * (bbuZ - tbuZ * 0.5) yang tidak pernah menyentuh tabel WHO mana pun.
 * Versi 2 memakai tabel BB/PB & BB/TB WHO yang sesungguhnya.
 */
export const Z_VERSI_BBTB = 2;

/** Penjaga agar satu pasien hanya ditulis ulang sekali per sesi. */
const sudahMigrasiBbtb = new Set<string>();

export interface HasilMigrasiBbtb {
  records: GrowthRecord[];
  /** Jumlah rekam yang bbtbZ-nya berhasil dihitung ulang dari tabel WHO. */
  dihitungUlang: number;
  /** Jumlah rekam yang bbtbZ-nya dibuang karena tidak bisa dipertanggungjawabkan. */
  dikosongkan: number;
  berubah: boolean;
}

/**
 * Menghitung ulang bbtbZ pada rekam lama memakai tabel WHO.
 *
 * Rekam yang sudah bertanda zVersi terkini dibiarkan apa adanya, sehingga fungsi
 * ini aman dipanggil berkali-kali.
 *
 * Bila jenis kelamin tidak diketahui, bbtbZ lama DIBUANG, bukan dipertahankan.
 * Nilai itu berasal dari rumus karangan; membiarkannya hanya membuat angka salah
 * tampak sah. Penanda versi sengaja tidak dipasang dalam kasus ini agar rekam
 * tersebut dihitung ulang begitu jenis kelaminnya diketahui.
 */
export function migrasiRekamBbtbZ(
  records: GrowthRecord[],
  jk?: Gender
): HasilMigrasiBbtb {
  let dihitungUlang = 0;
  ...
  const keluar = records.map((r) => {
    if (r.zVersi === Z_VERSI_BBTB) return r;

    if (!jk) {
      if (r.bbtbZ === undefined) return r;
      dikosongkan++;
      const sisa: GrowthRecord = { ...r };
      delete sisa.bbtbZ;
      return sisa;
    }

    const cara = r.caraUkur ?? caraUkurDariUsia(r.usiaBulan);
    const hasil = hitungZBbPbTb(r.tb, r.bb, jk, cara);
    const nilai = hasil && !hasil.diLuarTabel ? hasil.z : undefined;

    if (nilai === undefined) {
      dikosongkan++;
      const sisa: GrowthRecord = { ...r, caraUkur: cara, zVersi: Z_VERSI_BBTB };
      delete sisa.bbtbZ;
      return sisa;
    }

    dihitungUlang++;
    return { ...r, bbtbZ: nilai, caraUkur: cara, zVersi: Z_VERSI_BBTB };
  });
  ...
  return {
    ...
    dihitungUlang,
    dikosongkan,
    berubah: dihitungUlang + dikosongkan > 0,
  };
}
...
/**
 * Membaca rekam pertumbuhan dari localStorage.
 *
 * Tanpa jk, isinya dikembalikan APA ADANYA. Ini disengaja: pembanding di
 * pasangListenerRiwayat membandingkan hasil fungsi ini dengan data mentah
 * Firestore, sehingga memigrasi di sini akan membuat keduanya selalu berbeda
 * dan memicu tulis-ulang tanpa henti.
 */
export function loadGrowthRecords(patientId: string, jk?: Gender): GrowthRecord[] {
  ...
  try {
    ...
    const isi: GrowthRecord[] = Array.isArray(parsed) ? parsed : [];
    return jk ? migrasiRekamBbtbZ(isi, jk).records : isi;
  } catch {
    return [];
  }
}
...
export function useGrowthRecords(patientId: string, jk?: Gender): GrowthRecord[] {
  ...
  useEffect(() => {
    ...
    // Penghitungan ulang satu kali untuk rekam lama yang masih membawa bbtbZ
    // hasil rumus karangan. Aman diulang: rekam ber-zVersi terkini dilewati.
    const awal = migrasiRekamBbtbZ(loadGrowthRecords(patientId), jk);
    setRecords(awal.records);
    if (jk && awal.berubah && !sudahMigrasiBbtb.has(patientId)) {
      sudahMigrasiBbtb.add(patientId);
      saveGrowthRecords(patientId, awal.records);
    }

    const unsubFirebase = syncGrowthRecordsFromFirebase(patientId, (updatedRecords) => {
      setRecords(migrasiRekamBbtbZ(updatedRecords, jk).records);
    });

    const handleLocalChange = (e: Event) => {
      const customEv = e as CustomEvent<{ patientId?: string }>;
      if (!customEv.detail || customEv.detail.patientId === patientId) {
        setRecords(migrasiRekamBbtbZ(loadGrowthRecords(patientId), jk).records);
      }
    };
